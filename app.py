...
def susun_penulis(tabel_penulis, daftar_afiliasi, peta=None):
    ...
    if not penulis: catatan.append("Belum ada penulis yang diisi.")
    n_koresp = sum(1 for p in penulis if p.is_corresp)
    if n_koresp == 0: catatan.append("Belum ada penulis korespondensi yang ditandai.")
    elif n_koresp > 1: catatan.append("Lebih dari satu penulis korespondensi ditandai.")
    for i, teks in enumerate(daftar_afiliasi, 1):
        if not teks.strip(): catatan.append(f"Kotak Afiliasi {i} masih kosong.")
...
with tab1:
    c1, c2 = st.columns(2)
    with c1: judul_id = st.text_input(L("Judul Bahasa Indonesia", "Indonesian title"), "Keanekaragaman Ikan Gobi di Muara Sungai Tondano")
    with c2: judul_en = st.text_input(L("Title in English", "English title"), "Diversity of Goby Fish in Tondano River Estuary")
    ...
    for c in catatan_penulis: st.warning(c)
    c3, c4 = st.columns(2)
    with c3: telepon = st.text_input("Telepon korespondensi", "+62-8XXXXXXXXXX")
    with c4: email = st.text_input("Surel korespondensi", "nama@unsrat.ac.id")
...
with tab3:
    ...
    if int(n_eq) > 0:
        for i in range(int(n_eq)):
            ...
            if latex.strip(): eq_inputs[f'eq_{eq_num}'] = latex
