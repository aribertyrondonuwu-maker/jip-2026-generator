...
with tab0:
    ...
    for p in penulis_list:
        _tp_init_rows.append({
            "Nama": p.nama if hasattr(p, "nama") else "",
    ...
    tp_tabel_penulis = st.data_editor(
        ...
        column_config={"Korespondensi ★": st.column_config.CheckboxColumn()}
    )
    ...
    penulis_tp_list = [
        str(b.get("Nama", "")).strip()
        for b in as_records(tp_tabel_penulis)
        if str(b.get("Nama", "")).strip()
    ]

    for nama_p in penulis_tp_list:
        with st.expander(f"📝 {nama_p}", expanded=(nama_p == penulis_tp_list[0] if penulis_tp_list else False)):
            cols_credit = st.columns(3)
            selected_roles = []
            for idx_r, role in enumerate(CREDIT_ROLES):
                col_idx = idx_r % 3
                if cols_credit[col_idx].checkbox(role, key=f"tp_credit_{nama_p}_{idx_r}"):
                    selected_roles.append(role)
            extra_roles = st.text_input(
                "Tambahan (pisah koma)", key=f"tp_credit_extra_{nama_p}",
